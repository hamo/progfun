...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = wordOccurrencesLower(w.toLowerCase)

  def wordOccurrencesLower(w: Word): Occurrences = {
    w.groupBy(c => c).mapValues(l => l.length).toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((ch,tm), acc) => acc ::: ( for { comb <- acc
        n <- 1 to tm
      } yield (ch, n) :: comb )
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val tmp = for (xo <- x) yield {
      y find (o => o._1 == xo._1) match {
        case Some(r) => (xo._1, xo._2 - r._2)
        case None => xo
      }
    }
    tmp filter (o => o._2 > 0)
